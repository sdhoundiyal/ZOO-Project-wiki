...
If we were successful with the implementation before the end of the code sprint, the time was lacking for setting up a new server instance, including this new functionality. Since then, we have put this updated instance in place with this basic implementation of the chaining supported. There is no User Interface to display the result but you can use any available online viewer like this [one](https://geojson.io/) to view your result. Anyway, the nested process Centroid result was used as an input for the processing of the primary Buffer service, which was the goal of this implementation.
